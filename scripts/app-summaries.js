function get_team_score_summary(temp_question_count = -1) {
  ...
  if (!isUUIDSession(session)) {
    return [[t('table.team_name'),t('table.percent'),t('table.score'),t('table.placement'),t('table.earned_points'),t('table.total_points'), t('table.total_points_rounded'), t('table.percent_rounded'), t('table.score_rounded'), t('table.placement_rounded')]];
  }
...
function get_block_score_summary(temp_question_count = -1) {
  ...
  if (!isUUIDSession(session)) {
    return [[t('table.block_name'),t('table.percent'),t('table.score'),t('table.earned_points'),t('table.total_points')]];
  }
  
  let question_count;
  let team_count;
  let orderedQuestions = getOrderedQuestions(session);
  let orderedTeams = getOrderedTeams(session);
  let orderedBlocks = getOrderedBlocks(session);
  question_count = temp_question_count === -1 ? orderedQuestions.length : temp_question_count;
...
function get_team_and_block_score_summary(temp_question_count = -1) {
  ...
  if (!isUUIDSession(session)) {
    return [[t('table.team_name'),t('table.block_name'),t('table.percent'),t('table.score'),t('table.earned_points'),t('table.total_points')]];
  }
  
  let orderedQuestions = getOrderedQuestions(session);
  let orderedTeams = getOrderedTeams(session);
  let orderedBlocks = getOrderedBlocks(session);
  let question_count = temp_question_count === -1 ? orderedQuestions.length : temp_question_count;
  let team_names = orderedTeams.map(t => t.data.get('name') || '');
  let team_count = orderedTeams.length;
...
function get_question_log(temp_question_count = -1) {
  ...
  if (!isUUIDSession(session)) {
    return [[t('table.question'), t('score_entry.block_group'), t('table.possible_points'), t('table.ignore_question')]];
  }
