/**
 * English Language Pack for PBE Score Keeper
 * 
 * To add a new language, copy this file and:
 * 1. Rename to {language-code}.js (e.g., es.js for Spanish)
 * 2. Update the register_i18n_language() call with your language code
 * 3. Update the 'name' to your language's name in that language
 * 4. Update 'locale' to the appropriate locale code for date formatting
 * 5. Set 'rtl' to true if your language is right-to-left
 * 6. Translate all strings in the translations object
 * 7. Add your language code to i18n_available_language_codes in app-i18n.js
 */
register_i18n_language('en', {
  name: 'English',
  locale: 'en',
  rtl: false,
  translations: {
    "app": {
      "title": "PBE Score Keeper",
      "theme": "Theme",
      "language": "Language",
      "auto": "Auto"
    },
    "theme": {
      "system": "System",
      "light": "Light",
      "dark": "Dark"
    },
    "ui_mode": {
      "label": "UI Mode",
      "classic": "Classic",
      "default": "Default",
      "toggle_aria": "Toggle default UI",
      "toggle_title": "Switch between Classic and Default UI"
    },
    "menu": {
      "toggle_aria": "Menu",
      "toggle_title": "Menu"
    },
    "config": {
      "title": "Configuration (Quiz)",
      "session_title": "Quiz",
      "instructions_title": "Instructions",
      "instructions": "This is a score keeper for the Pathfinder Bible Experience (aka the Bible Bowl). Please enter your number of teams as well as blocks below so that the scoring grid can be created",
      "storage_title": "Data Storage Note",
      "storage_note": "Data is stored only on your device, and is not shared in any way with any server. This also means that if you change devices your data will not appear on the new device.",
      "new_session": "New Quiz",
      "enter_scores": "Enter Scores"
    },
    "session_nav": {
      "previous": "Previous Quiz",
      "next": "Next Quiz"
    },
    "teams": {
      "title_one": "Set up your Team",
      "title_other": "Set up your {{count}} Teams",
      "count_one": "{{count}} team",
      "count_other": "{{count}} teams",
      "team": "team",
      "teams": "teams",
      "name_label": "Team {{number}} Name:",
      "score_label": "{{name}}'s score",
      "score_label_s": "{{name}}' score",
      "add_team": "Add Team",
      "manage_button": "Manage Teams",
      "manage_title": "Manage Teams",
      "manage_close": "Close",
      "delete_team": "Delete Team",
      "delete_aria": "Delete {{name}}",
      "minimum_notice": "This team cannot be deleted because at least 1 team is required"
    },
    "blocks": {
      "title_one": "Set up your Block",
      "title_other": "Set up your {{count}} Blocks",
      "count_one": "{{count}} block",
      "count_other": "{{count}} blocks",
      "block": "block",
      "blocks": "blocks",
      "name_label": "Block {{number}} Name:",
      "add_block": "Add Block",
      "manage_button": "Manage Blocks",
      "manage_title": "Manage Blocks",
      "manage_close": "Close",
      "delete_block": "Delete Block",
      "delete_aria": "Delete {{name}}",
      "minimum_notice": "This block cannot be deleted because at least 1 block is required",
      "in_use_notice": "{{name}} cannot be deleted because it is assigned to questions"
    },
    "points": {
      "title": "Maximum Points per Question",
      "count_one": "{{count}} point",
      "count_other": "{{count}} points",
      "point": "point",
      "points": "points",
      "edit_max_points": "Click to edit maximum points",
      "possible": "Possible Points",
      "minimum_notice": "Cannot be lower due to already entered scores"
    },
    "rounding": {
      "title": "Live Score",
      "title_classic": "Rounding Live Team score to best team's total?",
      "yes": "Rounded",
      "no": "Exact"
    },
    "timer": {
      "title": "Question Timer",
      "enable": "Enable timer",
      "auto_start": "Auto start timer when score is selected",
      "toggle_label": "Timer",
      "enabled": "Enabled",
      "disabled": "Disabled",
      "first_point_seconds": "Seconds for first point",
      "subsequent_point_seconds": "Seconds for each additional point",
      "warning_flash_seconds": "Seconds remaining for warning flash",
      "time_remaining": "Time Remaining",
      "remove_time": "Remove 1 second",
      "add_time": "Add 1 second",
      "restart": "⏮️",
      "stop": "⏹️"
    },
    "score_entry": {
      "title": "Entry",
      "previous": "Previous Question",
      "next": "Next Question",
      "previous_short": "Previous Question",
      "next_short": "Next Question",
      "new": "New Question",
      "new_short": "New Question",
      "advanced_toggle": "Advanced Options",
      "ignore": "Ignore this Question in Score Calculations",
      "extra_credit": "Allow Extra Credit",
      "question": "Question",
      "question_title": "Question {{number}}",
      "team": "Team",
      "team_one": "Team",
      "team_other": "Teams",
      "block_group": "Block",
      "block_group_one": "Block",
      "block_group_other": "Blocks",
      "reorder_points": "Drag to reorder Possible Points field",
      "reorder_block": "Drag to reorder Block field",
      "reorder_entry": "Reorder Data Entry",
      "reorder_entry_hint": "Drag entries to reorder the data entry fields.",
      "reorder_entry_drag_aria": "Drag to reorder {{name}}"
    },
    "scores": {
      "according": "Results",
      "team": "Team",
      "team_exact": "Team (Exact)",
      "team_rounded": "Team (Rounded)",
      "by_block": "Block",
      "team_and_block": "Team by Block",
      "question_history": "Question Log"
    },
    "table": {
      "team_name": "Team Name",
      "percent": "Percent",
      "score": "Score",
      "placement": "Placement",
      "block_name": "Block Name",
      "question": "Question",
      "possible_points": "Possible Points",
      "earned_points": "Earned Points",
      "total_points": "Total Points",
      "total_points_rounded": "Total Points (Rounded)",
      "percent_rounded": "Percent (Rounded)",
      "score_rounded": "Score (Rounded)",
      "placement_rounded": "Placement (Rounded)",
      "ignore_question": "Ignore Question"
    },
    "placements": {
      "first": "First Place",
      "second": "Second Place",
      "third": "Third Place"
    },
    "edit_log": {
      "title": "Change Log",
      "change_log": "Change Log",
      "see_all_changes": "See All Changes",
      "close_button": "Close",
      "time": "Time",
      "user": "User",
      "session": "Quiz",
      "action": "Action",
      "details_header": "Details",
      "no_changes": "No changes recorded yet. Make some changes to see them here!",
      "global": "Global",
      "current_session": "Current Quiz",
      "unknown_time": "Unknown",
      "actions": {
        "change": "Change",
        "rename_session": "Rename Quiz",
        "add_team": "Add Team",
        "delete_team": "Delete Team",
        "rename_team": "Rename Team",
        "add_block": "Add Block",
        "delete_block": "Delete Block",
        "rename_block": "Rename Block",
        "change_max_points": "Change Max Points",
        "rename_question": "Rename Question",
        "change_rounding": "Change Rounding",
        "ignore_question": "Ignore Question",
        "include_question": "Include Question",
        "enable_extra_credit": "Enable Extra Credit",
        "disable_extra_credit": "Disable Extra Credit",
        "clear_extra_credit": "Clear Extra Credit",
        "extra_credit": "Extra Credit",
        "set_question_points": "Set Question Points",
        "change_question_block": "Change Question Block",
        "score_change": "Score Change",
        "export_session": "Export Current Quiz",
        "export_all_sessions": "Export All Quizzes",
        "reorder_teams": "Reorder Teams",
        "reorder_blocks": "Reorder Blocks",
        "merge": "Merge Quizzes",
        "restore_backup": "Restore Backup"
      },
      "details_templates": {
        "renamed": "Renamed \"{{old}}\" to \"{{new}}\"",
        "added": "Added \"{{name}}\"",
        "deleted": "Deleted \"{{name}}\"",
        "increased_max_points": "Increased max points from {{old}} to {{new}}",
        "auto_increased_max_points": "Auto-increased max points from {{old}} to {{new}} (score exceeded limit)",
        "decreased_max_points": "Decreased max points from {{old}} to {{new}}",
        "set_max_points": "Set max points from {{old}} to {{new}}",
        "enabled_rounding": "Enabled rounding to best team's total",
        "disabled_rounding": "Disabled rounding",
        "set_ignored": "Set \"{{name}}\" to be ignored",
        "set_included": "Set \"{{name}}\" to be included",
        "enabled_extra_credit": "Enabled extra credit for \"{{name}}\"",
        "disabled_extra_credit": "Disabled extra credit for \"{{name}}\"",
        "cleared_extra_credit": "Cleared all extra credit for \"{{name}}\"",
        "increased_extra_credit": "Increased extra credit for \"{{team}}\" on \"{{question}}\" to {{value}}",
        "decreased_extra_credit": "Decreased extra credit for \"{{team}}\" on \"{{question}}\" to {{value}}",
        "set_question_points": "Set max points for \"{{name}}\" from {{old}} to {{new}}",
        "changed_block": "Changed \"{{question}}\" from \"{{old}}\" to \"{{new}}\"",
        "score_changed": "\"{{team}}\" on \"{{question}}\" from {{old}} to {{new}}",
        "exported_session_yjs": "Exported quiz as .yjs file",
        "exported_session_json": "Exported quiz as JSON file",
        "exported_session_json_fallback": "Exported quiz as JSON file (fallback)",
        "exported_all_yjs": "Exported all quizzes as .yjs file",
        "exported_all_json": "Exported all quizzes as JSON file",
        "exported_all_json_fallback": "Exported all quizzes as JSON file (fallback)",
        "new_order": "New order: {{order}}",
        "merged_sessions": "Merged \"{{source}}\" into \"{{target}}\"",
        "merged_items": "Merged {{teams}} team(s), {{blocks}} block(s) from local data",
        "restored_backup": "Restored from backup ({{date}}, {{reason}})"
      }
    },
    "advanced": {
      "title": "Import/Export",
      "export_csv": "Export CSV",
      "export_team": "Export Score by Team",
      "export_block": "Export Score by Block",
      "export_team_and_block": "Export Score by Team by Block",
      "export_question_history": "Export Question History",
      "export_json": "Export for Importing",
      "export_session": "Export Current Quiz",
      "export_all": "Export All",
      "import": "Import",
      "import_warning": "Warning: Importing bad files can corrupt your data. It is strongly recommended that you run an \"Export for Importing\" before importing.",
      "select_file": "Please Select file to import:",
      "danger_zone": "Danger Zone",
      "merge": "Merge",
      "merge_sessions": "Merge Quizzes",
      "auto_merge": "Auto Merge Duplicates",
      "delete": "Delete",
      "delete_session": "Delete this Quiz",
      "no_import_support": "Your Browser does not support importing.",
      "device_migration": "Device Migration",
      "migration_description": "Transfer all quizzes between devices using peer-to-peer sync.",
      "migration_send": "Send to Another Device",
      "migration_receive": "Receive from Another Device",
      "migration_send_title": "Send Data to Another Device",
      "migration_receive_title": "Receive Data from Another Device",
      "migration_send_instructions": "Share this code with the receiving device:",
      "migration_receive_instructions": "Enter the code shown on the sending device:",
      "migration_room_code_label": "Migration Code",
      "migration_room_code_placeholder": "Enter 6-character code",
      "migration_start_send": "Start Sending",
      "migration_start_receive": "Start Receiving",
      "migration_cancel": "Cancel",
      "migration_waiting": "Waiting for other device to connect...",
      "migration_connected": "Connected! Transferring data...",
      "migration_progress": "Transferring quiz {{current}} of {{total}}: {{name}}",
      "migration_complete": "Migration complete! {{count}} quizzes transferred.",
      "migration_error": "Migration failed: {{error}}",
      "migration_no_sessions": "No quizzes to transfer.",
      "migration_close": "Close",
      "migration_info": "This uses the same peer-to-peer technology as Real-Time Sync. Both devices must be online.",
      "close_button": "Close",
      "pwa_tools": "PWA Tools",
      "pwa_tools_description": "Manage updates and view offline diagnostics for this installed app.",
      "pwa_status_title": "Offline Diagnostics",
      "pwa_refresh_status": "Refresh Diagnostics",
      "pwa_status_installed": "Installed as app",
      "pwa_status_online": "Network available",
      "pwa_status_service_worker": "Service worker registered",
      "pwa_status_service_worker_control": "Service worker controlling this page",
      "pwa_status_sync": "Sync state",
      "pwa_status_periodic_sync": "Periodic background sync",
      "pwa_status_last_update_check": "Last update check",
      "pwa_status_yes": "Yes",
      "pwa_status_no": "No",
      "pwa_status_unknown": "Unknown",
      "pwa_status_never": "Never",
      "install_app": "Install as App",
      "install_success": "App installed successfully.",
      "install_instructions_ios": "To install: tap Share in Safari, then Add to Home Screen.",
      "install_instructions_android": "To install: open browser menu, then tap Install app or Add to Home screen.",
      "install_instructions_desktop": "To install: use the install icon in the browser address bar or browser menu.",
      "check_updates": "Check for Updates",
      "reload_app": "Reload App",
      "reload_now": "Reload Now",
      "clear_cache": "Clear Cache & Restart",
      "clear_cache_confirm": "This clears app file cache and restarts the app. Saved quizzes are stored separately.",
      "update_available": "App update is available! Reload to apply.",
      "update_ready_deferred": "Update is ready. It will apply when the app is idle or reopened.",
      "checking_updates": "Checking for updates...",
      "no_update_available": "You're running the latest version.",
      "cache_cleared": "Cache cleared. Restarting app..."
    },
    "footer": {
      "feedback": "Have an idea to make this better?",
      "let_me_know": "Let me know",
      "version": "v {{version}}"
    },
    "placeholders": {
      "team_scores": "Team Scores Go Here",
      "rounded_scores": "Rounded Team Scores Go Here",
      "block_scores": "Block Scores Go Here",
      "team_block_scores": "Team by Block Scores Go Here",
      "question_history": "Question History Goes Here"
    },
    "defaults": {
      "session_name": "{{date}}",
      "team_name": "Team {{number}}",
      "block_name": "Block {{number}}",
      "no_block": "No Block",
      "question_name": "Question {{number}}",
      "extra_credit": "Extra Credit",
      "unnamed_session": "Unnamed Quiz"
    },
    "confirm": {
      "delete_team": "Do you really want to delete {{name}}?",
      "delete_block": "Do you really want to delete {{name}}?",
      "delete_extra_credit": "Are you sure you want to irreversibly delete this question's extra credit?",
      "delete_session": "Are you sure you want to irreversibly delete \"{{name}}\"?"
    },
    "alerts": {
      "migration_failed": "Data migration failed. Your data is safe in localStorage. Please export a backup and report this issue.",
      "cannot_delete_only_session": "You may not delete the only Quiz",
      "import_success": "Import successful: {{count}} quiz(es) imported",
      "import_failed": "Import failed: {{error}}",
      "import_json_failed": "Failed to import JSON file: {{error}}",
      "import_binary_failed": "Failed to import binary file: {{error}}",
      "select_valid_file": "Please select json or yjs file for import",
      "loading": "Loading...",
      "unknown_import_error": "Unknown import error",
      "yjs_not_initialized": "Yjs not initialized",
      "invalid_import_format": "Invalid import format. Expected JSON or binary (.yjs) file.",
      "failed_parse_container": "Failed to parse multi-doc container",
      "failed_merge_global": "Failed to merge global state: {{error}}",
      "failed_import_session_id": "Failed to import quiz {{id}}: {{error}}",
      "failed_import_session": "Failed to import quiz: {{error}}",
      "failed_convert_localstorage": "Failed to convert localStorage format: {{error}}"
    },
    "history_global": {
      "actions": {
        "create_session": "Create Quiz",
        "switch_session": "Switch Quiz",
        "delete_session": "Delete Quiz",
        "merge_session": "Merge Quiz",
        "reorder_sessions": "Reorder Quizzes",
        "import": "Import"
      },
      "details_templates": {
        "created_session": "Created \"{{name}}\"",
        "created_synced_session": "Created synced quiz \"{{name}}\"",
        "switched_session": "Switched to \"{{name}}\"",
        "deleted_session": "Deleted \"{{name}}\"",
        "merged_session": "Merged \"{{source}}\" into \"{{target}}\"",
        "reordered_sessions": "Changed quiz order",
        "imported_sessions": "Imported {{count}} quiz(es)",
        "imported_from_json": "Imported {{count}} quiz(es) from JSON file",
        "imported_from_yjs": "Imported {{count}} quiz(es) from .yjs file"
      }
    },
    "sync": {
      "button": "Sync",
      "title": "Collaborate",
      "button_tooltip": "Collaborate in real-time with others",
      "status_offline": "Offline",
      "status_connecting": "Connecting...",
      "status_connected": "Connected",
      "status_error": "Connection Error",
      "has_sync_room": "This quiz has been synced",
      "peer_count_one": "{{count}} person editing",
      "peer_count_other": "{{count}} people editing",
      "peers_tooltip": "Connected: {{names}}",
      "dialog_title": "Real-Time Sync",
      "mode_selection": "Choose sync mode",
      "display_name_label": "Your Display Name",
      "display_name_placeholder": "Enter your name",
      "create_room": "Create New Room",
      "join_room": "Join Existing Room",
      "room_code_label": "Room Code",
      "room_code_placeholder": "Enter 6-character code",
      "room_code_hint": "Get this code from the room creator",
      "use_password": "Protect room with password",
      "password_label": "Room Password",
      "password_placeholder": "Enter password",
      "password_hint": "All participants must use the same password",
      "password_required": "This room requires a password",
      "password_incorrect": "Incorrect password. Please try again.",
      "connect_button": "Connect",
      "disconnect_button": "Disconnect",
      "cancel_button": "Cancel",
      "share_instructions": "Share this code with others to let them join:",
      "share_password_note": "This room is password-protected. Share the password separately.",
      "your_name_label": "You:",
      "edit_name": "Edit",
      "edit_name_title": "Change your display name",
      "save_name": "Save name",
      "name_updated": "Display name changed to {{name}}",
      "invalid_room_code": "Invalid room code. Please enter 6 characters.",
      "invalid_display_name": "Please enter your display name.",
      "connection_failed": "Failed to connect. Please try again.",
      "disconnected": "Disconnected from sync room.",
      "name_changed": "Your display name was changed to {{name}} to avoid duplicates",
      "peer_joined": "{{name}} joined",
      "peer_left": "{{name}} left",
      "join_choice_title": "Join Quiz",
      "join_choice_description": "This room has an existing quiz. What would you like to do?",
      "join_choice_new_session": "Create new local quiz (recommended)",
      "join_choice_new_session_desc": "Start fresh with the room's data in a new quiz",
      "join_choice_merge_session": "Merge into current quiz",
      "join_choice_merge_session_desc": "Combine room data with your current quiz",
      "matching_title": "Match Teams & Blocks",
      "matching_description": "The room has different names. Please confirm how to match them:",
      "matching_teams_header": "Teams",
      "matching_blocks_header": "Blocks/Groups",
      "matching_questions_header": "Questions",
      "matching_remote": "Remote",
      "matching_local": "Your Local",
      "matching_create_new": "➕ Create New",
      "matching_auto_matched": "Auto-matched",
      "matching_confirm": "Confirm & Sync",
      "matching_skip": "Skip (Don't Sync)",
      "history_local_user": "(local)",
      "history_unknown_user": "(unknown)",
      "version_mismatch_warning": "Warning: {{peers}} may be using an older version (requires v{{minVersion}}+). Data may not sync correctly.",
      "version_mismatch_disconnect": "Disconnected: {{peers}} are using an older version (requires v{{minVersion}}+).",
      "copy_code": "Copy Code",
      "code_copied": "Room code copied to clipboard",
      "error_timeout": "Connection timed out. Please try again.",
      "error_network": "Network error. Check your connection.",
      "error_server_down": "Sync servers unavailable. Try again later.",
      "error_session_deleted": "Quiz was deleted. Disconnecting from sync.",
      "error_peer_left": "{{name}} disconnected.",
      "reconnecting": "Reconnecting...",
      "reconnect_failed": "Failed to reconnect. Click to retry.",
      "session_switch_disconnect": "Switched quizzes. Disconnected from sync room.",
      "auto_reconnected": "Reconnected to room {{code}}",
      "switch_confirm_title": "Leave Sync Room?",
      "switch_confirm_message": "You are currently syncing with room {{code}}. Switching quizzes will disconnect you from the sync room.",
      "switch_confirm": "Switch Quiz",
      "switch_cancel": "Stay Connected",
      "aria_sync_status": "Sync status: {{status}}",
      "aria_peer_count": "{{count}} people currently editing",
      "aria_peer_joined": "{{name}} has joined the quiz",
      "aria_peer_left": "{{name}} has left the quiz",
      "aria_room_created": "Room created with code {{code}}",
      "info_link_title": "Learn about sync limitations",
      "info_dialog_title": "About Real-Time Sync",
      "info_dialog_p2p": "This feature uses peer-to-peer (P2P) sync technology, which works best for small to medium-sized groups.",
      "info_dialog_limit": "Recommended limit: 20-30 simultaneous users per room.",
      "info_dialog_larger": "Need to support a larger group?",
      "info_dialog_large_link": "Click here to create a large group sync",
      "info_dialog_close": "Close",
      "large_dialog_title": "Large Event Sync",
      "large_dialog_description": "Server-based sync for large groups (30+ participants). Uses encrypted connection for secure data transfer.",
      "large_mode_label": "Create or Join Room",
      "large_mode_create": "Create New Room",
      "large_mode_join": "Join Existing Room",
      "large_room_help": "Enter the room code shared by the host (e.g., L-ABC123)",
      "large_password_help": "Password is required. Share it securely with participants.",
      "large_expiry_notice": "Large event rooms expire at midnight. You will need to create a new room for the next day.",
      "large_button_connect": "Connect",
      "large_room_created": "Room created! Share code: {{code}}",
      "server_badge_tooltip": "Connected via server (large event mode)",
      "error_timeout": "Connection timed out. Please try again.",
      "large_event_title": "Large Event Sync",
      "large_event_desc": "Server-based sync for 30+ participants. Uses encrypted connection.",
      "large_event_note": "Note: Room data is stored temporarily on the server during your event. Rooms are cleared when the server restarts (typically within 24 hours).",
      "large_event_password_required": "Password is required for large event rooms",
      "large_event_connecting": "Connecting to server...",
      "large_event_connected": "Connected via server",
      "large_event_expired": "Large event room expired",
      "large_event_expired_desc": "Large event rooms expire at midnight. Create a new room for today's event.",
      "server_mode_badge": "Server",
      "room_code_large_prefix": "L-",
      "error_room_exists": "Room already exists. Try joining instead.",
      "error_room_not_found": "Room not found. Check the code or create a new room.",
      "error_invalid_password": "Invalid password for this room.",
      "error_server_unavailable": "Server unavailable. Please try again later.",
      "join_choice_join_session": "Join Quiz",
      "join_choice_join_session_desc": "Receive data from the synced room (recommended)",
      "join_advanced_options": "Advanced Options",
      "join_merge_warning": "Only use merge if you understand that it adds your local teams/blocks to the synced quiz.",
      "join_button": "Join",
      "receiving_data": "Receiving quiz data...",
      "synced_session_name": "Synced Quiz {{date}}",
      "merge_nothing_to_add": "All items already exist in the synced quiz",
      "merge_cancelled": "Merge cancelled",
      "merge_complete": "Merged {{teams}} team(s) and {{blocks}} block(s)",
      "merge_preview_title": "Merge Preview",
      "merge_preview_description": "The following items will be added to the synced quiz:",
      "merge_teams_to_add": "Teams to add ({{count}}):",
      "merge_blocks_to_add": "Blocks to add ({{count}}):",
      "merge_none": "(none)",
      "merge_backup_note": "A backup will be created before merging.",
      "merge_confirm_button": "Merge Items",
      "room_collision_error": "Network busy. Try adding a password for a private room.",
      "registry_unavailable": "Unable to connect to sync server. Please try again, or add a password to create a private room.",
      "room_not_found": "Room not found. Check the code and try again.",
      "room_requires_password": "This room requires a password. Please enter the password shared by the room creator."
    },
    "backup": {
      "reason_pre_merge": "Before Merge",
      "reason_pre_restore": "Before Restore",
      "reason_pre_import": "Before Import",
      "reason_pre-migration": "Before Upgrade",
      "reason_manual": "Manual Backup",
      "reason_auto": "Auto Backup",
      "restored_success": "Quiz restored from backup",
      "restore_failed": "Failed to restore from backup",
      "view_backups": "View Backups",
      "browser_title": "Quiz Backups",
      "browser_description": "Backups for \"{{session}}\"",
      "no_backups": "No backups available for this quiz",
      "column_date": "Date",
      "column_reason": "Reason",
      "column_summary": "Contents",
      "column_actions": "Actions",
      "summary_format": "{{teams}} teams, {{questions}} questions, {{blocks}} blocks",
      "restore_button": "Restore",
      "restore_title": "Restore this backup",
      "export_button": "Export",
      "export_title": "Export backup for import on another device",
      "pin_title": "Pin to prevent auto-deletion",
      "unpin_title": "Unpin to allow auto-deletion",
      "delete_title": "Delete this backup",
      "create_manual": "Create Backup Now",
      "close_button": "Close",
      "confirm_restore": "Restore this backup? A backup of the current state will be created first.",
      "confirm_delete": "Delete this backup? This cannot be undone.",
      "created_success": "Backup created",
      "exported_success": "Backup exported"
    },
    "session_manager": {
      "dialog_title": "Manage Quizzes",
      "description": "Rename, reorder, or delete quizzes",
      "name_column": "Quiz Name",
      "actions_column": "Actions",
      "delete_aria": "Delete {{name}}",
      "drag_aria": "Drag to reorder {{name}}",
      "close_button": "Close",
      "rename_placeholder": "Enter quiz name",
      "empty_name_error": "Quiz name cannot be empty",
      "manage_button": "Manage Quizzes",
      "reorder_hint": "Drag quizzes to reorder"
    },
    "session_dialogs": {
      "continue_or_new_title": "Welcome Back!",
      "continue_or_new_message": "Would you like to continue with your previous quiz or start fresh with a new one?",
      "continue_session": "Continue Previous",
      "start_new_session": "Start New",
      "current_session_label": "Previous:"
    },
    "merge": {
      "dialog_title": "Merge Quizzes",
      "select_source": "Select Source Quiz",
      "select_target": "Select Target Quiz",
      "source_help": "Data from this quiz will be merged into the target",
      "target_help": "This quiz will receive the merged data",
      "merge_button": "Merge",
      "cancel_button": "Cancel",
      "same_session_error": "Please select two different quizzes",
      "no_sessions_error": "You need at least two quizzes to merge",
      "merge_success": "Quizzes merged successfully",
      "merge_failed": "Merge failed: {{error}}",
      "matching_source": "Source",
      "matching_target": "Target",
      "confirm_merge": "Confirm Merge",
      "auto_merge_no_duplicates": "No duplicate quizzes found to merge",
      "auto_merge_processing": "Merging {{count}} duplicate quiz(es)...",
      "auto_merge_complete_title": "Auto Merge Complete",
      "auto_merge_summary": "Successfully merged {{count}} duplicate quiz(es):",
      "source_session": "Source Quiz",
      "target_session": "Merged Into",
      "ok_button": "OK",
      "auto_merge_failures": "{{count}} merge(s) failed:",
      "failure_reason": "Error",
      "empty_sessions_deleted": "{{count}} empty duplicate quiz(es) deleted:",
      "empty_session_deleted": "Deleted (no data)",
      "status": "Status"
    },
    "pwa": {
      "file_launch_success": "Imported quiz from file",
      "file_launch_error": "Failed to import file: {{error}}",
      "file_launch_invalid": "Invalid file launch data",
      "file_reader_unavailable": "File reading is not available in this browser",
      "file_read_failed": "The file could not be read",
      "unsupported_file_type": "Only .yjs and .json files can be imported",
      "import_unavailable": "Import is not available yet",
      "unknown_import_error": "Unknown import error",
      "http_error": "HTTP {{status}}",
      "protocol_join_sync": "Joining sync room {{code}}...",
      "protocol_new_session": "Creating new quiz...",
      "protocol_import_url": "Importing quiz from link...",
      "protocol_error_invalid": "Invalid protocol link",
      "protocol_error_missing_param": "Missing required parameter: {{param}}"
    }
  }
});
